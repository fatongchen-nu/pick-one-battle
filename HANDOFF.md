...
> 本轮改动（战报改为深色淘汰赛风并保证竖屏、同人女极限二选一署名 xhs@degree、用户投稿改为自动公开）
> 尚在本地，请在本机终端 `git push origin main`；「自动公开」还需在 Supabase 执行下方「上线步骤」里的 RLS 策略。

## 1. 产品目标

这是一个无需手填长图的「极限二选一」淘汰赛网站。用户选择题库后，从初始名单逐轮二选一，最终得到唯一冠军，并可查看完整晋级树、下载**深色竖屏淘汰赛战报**或分享永久结果页面。
...
- `app.js`：内置题库、淘汰赛状态、撤回、对阵树、分享、深色竖屏淘汰赛战报绘制、内嵌二维码。
...
- 游戏中查看当前对阵树。
- 结果页展示完整晋级路线。
- **下载深色竖屏淘汰赛战报（PNG）**：懂球帝风深色底，选手从上下两端向中央的冠军收拢；
  晋级者橙色描边高亮、被淘汰者变暗，左侧标注轮次（16强/8强/半决赛/决赛），中央金橙冠军框带发光。
  **保证竖屏**：算出宽高后若偏横，自动缩窄格宽、拉大行距直到画面变竖（`downloadCard()` 里的 geom/guard 循环，
  4—128 项均已实测竖屏）。文字自适应缩放不溢出；底部含**二维码 + 网址**（深色底加白垫确保可扫），扫码直达网站。
...
- 有 4 个 `xhs@` 投稿题库保留作者（含同人女极限二选一 = `xhs@degree`）；其余 6 个内置题库不署名（`author` 字段缺省 / 数据库 `author_name` 为 `null`）。
...
1. 在 Supabase SQL Editor 中整段执行 `supabase-schema.sql`。可在已有数据库上安全重复运行：
   升级至 128 项、重建所需 RLS policies（**投稿策略已改为允许直接写入 `approved`**）、同步 10 套内置题库；
...
1. 首页能显示全部 10 套题库；6 个不显示作者，4 个 `xhs@` 正常显示（含同人女 = `xhs@degree`）。
...
6. 复制的永久链接可在另一浏览器打开。
7. 下载战报：深色底、冠军居中不溢出、晋级路线橙色高亮、被淘汰者变暗、左侧轮次标签、**输出为竖屏（高 ≥ 宽）**、底部二维码可扫（解码到线上地址）、文字不裁切。
...
以及用 node-canvas 对深色竖屏战报（4/14/16/32/48/64/128 项，均验证输出为竖屏、含二维码）实跑出图验证。
...
- 战报现为深色淘汰赛风并**保证竖屏**：大题库会自动缩窄格宽、拉大行距（128 项约 3300×3500px），不再横向铺开。
