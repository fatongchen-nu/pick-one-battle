...
-- 允许在已经初始化过的项目中重复执行本文件，并将旧的 64 项限制升级为 128 项。
...
drop policy if exists "read approved public brackets" on public.brackets;
...
drop policy if exists "read shared results" on public.results;
...
values
  (
    ...
    'xhs@degree', 'public', 'approved', 47, '{"宿敌变情人":16,"Hurt / Comfort":11,"青梅竹马":9,"欢喜冤家":6,"灵魂互换":5}'::jsonb
  ),
  (
    ...
    null, 'public', 'approved', 38, '{"Wait for It":13,"Satisfied":10,"Burn":8,"Non-Stop":4,"My Shot":3}'::jsonb
  ),
  (
    ...
    null, 'public', 'approved', 24, '{"Turn It Off":9,"I Believe":7,"Hello!":5,"Spooky Mormon Hell Dream":3}'::jsonb
  ),
  (
    ...
    null, 'public', 'approved', 16, '{"Waving Through a Window":6,"You Will Be Found":5,"Words Fail":3,"For Forever":2}'::jsonb
  ),
  (
    ...
    null, 'public', 'approved', 29, '{"Monster":8,"Ruthlessness":6,"God Games":5,"Warrior of the Mind":5,"Different Beast":5}'::jsonb
  ),
  (
    ...
    null, 'public', 'approved', 33, '{"洛基 Loki":9,"死侍 Wade Wilson":7,"蜘蛛侠 Peter Parker":6,"绯红女巫 Wanda Maximoff":6,"钢铁侠 Tony Stark":5}'::jsonb
  ),
  (
    ...
    null, 'public', 'approved', 41, '{"螺蛳粉":10,"肉夹馍":8,"煎饼果子":8,"生煎包":8,"豆花":7}'::jsonb
  ),
  (
    ...
    'xhs@一只不眠喵', 'public', 'approved', 21, '{"白切黑":7,"病弱":5,"傲娇":4,"美人":3,"冰山":2}'::jsonb
  ),
  (
    '55555555-5555-4555-8555-555555555555', '同人女左位 XP 二选一', '⬅️', '48 种角色属性 · 选出你的左位终极 XP',
    '["轻浮佬","重力系","小太阳","冰山","傲娇","直球","高敏","娇纵","黑切白","白切黑","天然呆","三无","心机","嘴毒","病弱","身强体壮","美人","糙汉","失眠症","嗜睡","中二病","电竞","恋旧","喜新厌旧","黏人","回避","温和","无情","冷脸萌","热脸蛋","成熟","幼稚","高情商","低情商","s","m","自私","无私","隐忍","放纵","自恋","自卑","绿茶","海王","颜控","醋精","霸总","暴君"]'::jsonb,
    'xhs@一只不眠喵', 'public', 'approved', 19, '{"黑切白":6,"糙汉":5,"直球":4,"身强体壮":2,"暴君":2}'::jsonb
  ),
  (
    ...
    'xhs@没有wifi也没有流量', 'public', 'approved', 26, '{"少爷和我":7,"史密斯再就业":6,"年会必须停":5,"回音山谷":4,"最后的铸剑师":4}'::jsonb
  ),
  (
    'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb', '同人病状/异常状态XP二选一', '🩹', '48 种病状与异常状态 · 选出你的终极 XP',
    '["头痛","痉挛","言语障碍","呼吸困难","嗜睡","失眠","虚脱","窒息","记忆混乱","泪失禁","呕吐","吐血","痛觉放大","无痛症","丧失听觉","丧失视觉","被害妄想","失去自主","晕厥","幻听","失温","创伤应激","自毁","毁他","流鼻血","饥饿","情感麻木","焦虑症","抑郁症","替身妄想","异食癖","感知停滞","施虐欲","皮肤饥渴","精神分裂","药物依赖","夜盲症","感官过载","情绪失控","性亢奋","狂躁症","畏光","恐慌症","异常兴奋","厌食症","耳鸣","幸福的幻觉","痛苦的幻觉"]'::jsonb,
    'ZN', 'public', 'approved', 15, '{"失去自主":5,"皮肤饥渴":3,"泪失禁":3,"被害妄想":2,"幻听":2}'::jsonb
  )
